export default {
  async register({commit}, credentials){
    try {
      const response = await axios.post('/register/', credentials) //returns the desired data as jason
  ...
  async login({commit}, credentials){
    try {
      const response = await axios.post('/login/', credentials) //returns the desired data as jason
      //use .data to extract the actual data from the request. 
      commit("SET_USER_DATA", response.data) //Verify response json to make sure of the structure.
    } catch (error) {
      console.log("Trouble fetching events.", error)
